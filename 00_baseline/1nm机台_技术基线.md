...
- 高精度有效量测FOV：200 µm × 200 µm；
- Die尺寸：10 mm × 10 mm；
- 视场重叠：约10%；
- 等效步距：约180 µm；
- 单Die视场数：约56 × 56 = 3136 FOV；
- micro-step move-to-Measurement-Ready：约50 ms/FOV；
- 单FOV曝光/采集/读出：暂按约10 ms/FOV；
- 数据处理优先按流水处理考虑，不默认完全阻塞运动。

由此：

```text
T_FOV ≈ 50 ms + 10 ms = 60 ms
```

单Die：

```text
T_Die ≈ 3136 × 60 ms ≈ 188 s ≈ 3.1 min
```

25 Die核心扫描时间：

```text
T_25Die ≈ 25 × 188 s ≈ 4704 s ≈ 78.4 min
```

计入上片、找正/建系、Wafer级调平、Die间寻址、全局IPD计算及下片后：

```text
T_Wafer ≈ 80–85 min
```

该结果为当前第一版工程估算，不作为已验证设备节拍。

### 9.2 节拍优化方向

当前已识别的主要节拍杠杆：

1. **扩大高精度有效FOV**：200 µm → 250–300 µm；
2. **缩短micro-step move-to-ready**：50 ms → 20–30 ms；
3. **缩短单FOV曝光/读出时间**，并尽量与平台动作、数据处理流水化重叠；
4. Die间大步距运动不是当前节拍主导项。

在FOV扩大至250–300 µm、micro-step move-to-ready压缩至20–30 ms后，单Wafer节拍预计可进一步压缩至约：

```text
30–50 min / wafer
```

该区间为节拍优化目标，不作为当前已实现能力。

### 9.3 当前节拍主矛盾

🟢 当前整机节拍主导项为Die内高密度FOV扫描，而非25个Die之间的大步距寻址。

对当前方案而言，单FOV节拍每降低10 ms，整片约78400个FOV可累计节省约13 min，因此micro-step move-to-ready与高精度有效FOV是优先级最高的节拍指标。
